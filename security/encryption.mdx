...
{/* TODO: confirm whether TLS 1.2 is intended to remain supported long-term, or whether TLS 1.3-only is the planned production stance — the sidecar relies on rustls defaults which currently allow both. */}
...
{/* TODO: explicit key-rotation policy for the disk encryption key and the backup encryption key is not documented in the repo. Today the keys are deterministic per attested identity (rotating means rotating the identity — i.e. cutting a new CVM image). Confirm with the team whether automated rotation is planned. */}
